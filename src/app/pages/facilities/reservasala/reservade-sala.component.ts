...
@Component({
  selector: 'app-reservade-sala',
  templateUrl: './reservade-sala.component.html',
  styleUrls: ['./reservade-sala.component.scss']
})
export class ReservadeSalaComponent implements OnInit {

  nameContent: string = 'Reserva de Salas';//my-toolbar
   ...
   */
  displayedColumns: string[] = ['code', 'lastSectorSent', 'nameClient', 'clientContactDateView', 'statusColumnTable','status'];//nome da coluna do json que será percorrido
  dataSource: any;//o conteudo da requisição com o Json do objeto retornado na consulta ao backend
  displayHeaderColumns: string[] = ['CODIGO', 'SALA', 'SOLICITANTE', 'RESERVA', 'LIBERAÇÃO','STATUS'];//nome da coluna que será exibido no html usado como rotulo
