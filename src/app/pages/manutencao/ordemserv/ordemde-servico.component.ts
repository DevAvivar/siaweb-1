...
@Component({
  selector: 'app-ordemde-servico',
  templateUrl: './ordemde-servico.component.html',
  styleUrls: ['./ordemde-servico.component.scss']
})
export class OrdemdeServicoComponent implements OnInit {

  nameContent: string = 'Ordem de Serviço';//my-toolbar
   ...
   */
  displayedColumns: string[] = ['code', 'lastSectorSent', 'nameClient', 'clientContactDateView','lastSectorSent','nameItem','nameUser','clientContactDateView','clientContactDate','status'];//nome da coluna do json que será percorrido
  dataSource: any;//o conteudo da requisição com o Json do objeto retornado na consulta ao backend
  displayHeaderColumns: string[] = ['CODIGO', 'EQUIPAMENTO', 'TAREFA', 'PLANO DE MANUTENÇÃO','LOCAL','IMPRESSO','Nº SOLICITAÇÃO','D.SOLICITAÇÃO','D.AGENDAMENTO','STATUS'];//nome da coluna que será exibido no html usado como rotulo
