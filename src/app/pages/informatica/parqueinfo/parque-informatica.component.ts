...
@Component({
  selector: 'app-parque-informatica',
  templateUrl: './parque-informatica.component.html',
  styleUrls: ['./parque-informatica.component.scss']
})
export class ParqueInformaticaComponent implements OnInit {

  nameContent: string = ' Pesquisa de Equipamentos do Parque de Informática';//my-toolbar
  ...
  displayHeaderColumns: string[] = ['IP', 'SETOR', 'UNIDADE', 'TIPO', 'MARCA MODELO'];//nome da coluna que será exibido no html usado como rotulo
  ...
  constructor(
      ...
      private productService: SacProductService,
 ...
 */
  ...
  listSac() {
      ...
      this.sac$.subscribe({
          next: (item) => {
              this.dataSource = new _MatTableDataSource(JSON.parse(JSON.stringify(item)).content);
  ...
  getProduct() {
      ...
      this.productService.findId(value_).subscribe({
          next: (item) => {
            console.log(item)
              if (item)
                  this.nameProduct.nativeElement.value = item.description;
              else {
...
*/
  fillClassification(): string {
    ...
    editar() { /* TODO document why this method 'limpar' is empty */ }
    remove() { /* TODO document why this method 'print' is empty */ }
    limpar() { /* TODO document why this method 'print' is empty */ }
    visualizar(){ /* TODO document why this method 'print' is empty */}
